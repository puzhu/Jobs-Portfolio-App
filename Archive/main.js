...
function draw(dataAll, rowLabelData, colLabelData) {
  ...
  */
  ...
  var gridPosition = gridData(mainRowLabelState, mainColLabelState, heatmapWidth, heatmapHeight,rowLabelData, colLabelData, dataAll)
  ...
  */
  function drawColLabels(mainColState, currChartData){
    ...
    var mainColPosData = setMainColLabelPosition(mainColState, heatmapWidth, innerHeight, outerHeight, colLabelData)
    ...
    if(mainColState.filter(function(d) {return d.expand}).length > 0){ //if expanded rotate
      ...
      var innerLabelPositions = setSublabelColPosition(mainColPosData, colLabelData, innerHeight)
    ...
    function onClickCols(d){
      ...
      //update the grid
      gridPosition = gridData(mainRowLabelState, mainColLabelState, heatmapWidth, heatmapHeight, rowLabelData, colLabelData, currChartData)
  ...
  function drawRowLabels(mainRowLabelState, currChartData){
    ...
    var innerRowSVG = innerRowChars.plotVar.attr("id", "innerRowSVG"),
        innerRowWidth = innerRowChars.width,
        outerRowSVG = outerRowChars.plotVar.attr("id", "outerRowSVG"),
        outerRowWidth = outerRowChars.width

    var rowPosData = setMainRowLabePosition(mainRowLabelState, heatmapHeight, innerRowWidth, outerRowWidth, rowLabelData) //sets the position using width argument that relies on the div that it is being attached to.
    ...
    var innerRowLabels = innerRowSVG.append('g').selectAll(".mainRowLabels")
      ...
      //Draw the inner labels
      var innerLabelPositions = setSubRowLabelPosition(rowPosData, rowLabelData, innerRowWidth)
  ...
  */
  ...
  function onChangeFunction(){
    ...
    gridPosition = gridData(mainRowLabelState, mainColLabelState, heatmapWidth, heatmapHeight, rowLabelData, colLabelData, tempData)
  ...
  */
  ...
  d3.select("#saveChartButton").on("click", function(){
    ...
    saveSvgAsPng(document.getElementById("innerRowSVG"), "innerRowLabels.png")
    saveSvgAsPng(document.getElementById("outerRowSVG"), "outerRowLabels.png")
