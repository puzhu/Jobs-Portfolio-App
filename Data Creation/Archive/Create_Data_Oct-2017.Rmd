...
appData <- read_excel("raw data Oct/BBL_Data_Visualization Tool_M&E All Outputs.xlsx", sheet = "M & E_Outputs", skip = 2)
names(appData)
...
bangladesh <- appData %>% 
        select(Proj.Country, 230:242) %>% 
        filter(Proj.Country == "Bangladesh" & !is.na(Intervention)) %>% 
        gather(key = subColLabel, value = count, 3:14) %>% 
        filter(!is.na(count)) %>% 
        mutate(count = 1)

bangaladeshCompare <- countData %>% 
        filter(Proj.Country == "Bangladesh") %>% 
        select(Proj.Country, Intervention, subColLabel, count)
...
write_csv(appData, "../data/appData-Oct-2017.csv")
